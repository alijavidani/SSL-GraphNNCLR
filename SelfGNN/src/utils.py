...
class Augmentations:
    ...
    def __init__(self, method='gdc'):
        methods = {"ppr", "heat", "ldp", "paste", "split", "zscore", "katz", None}
...
def parse_args(check_for_tuned=True):
    ...
    parser.add_argument("--name", "-n", type=str, default="Imagenet100",#Imagenet1K, Cifar10
                        help="Name of the dataset. Supported names are: cora, citeseer, pubmed, photo, computers, cs, and physics")
    parser.add_argument('--model', type=str, default='gcn', choices=['gcn', 'sage', 'gat', 'gin', 'gcnii'],
                        help='GNN type including GCNII (deep residual GCN)')
    parser.add_argument("--aug", '-a', type=str, default=None,
    ...
    parser.add_argument("--norms", "-nm", nargs="+", default=['batch',], 
                        ...
                        "prediction head. Finally, specifying three inputs, e.g., ['no', 'batch', 'layer', 'pair'] activates the projection head and "
    ...
    parser.add_argument("--layers", "-l", nargs="+", default=[
                        768, 768], help="The number of units of each layer of the GNN. Default is [512, 128]")
    parser.add_argument("--init-parts", "-ip", type=int, default=40,
                        help="The number of initial partitions. Default is 1. Applicable for ClusterSelfGNN")
    parser.add_argument("--final-parts", "-fp", type=int, default=30,
    ...
    parser.add_argument("--heads", '-hd', nargs="+", type=int, default=[
                        1, 1], help="The number of heads of each layer of a GAT architecture. Default is [8, 1]. Applicable for gat model.")
    parser.add_argument("--lr", '-lr', type=float, default=0.0001,
                        help="Learning rate. Default is 0.0001.")
    parser.add_argument("--dropout", "-do", type=float,
                        default=0, help="Dropout rate. Default is 0.2")
    ...
    parser.add_argument("--graph_type", '-gt', type=str, default='both', help="The graph type used for training. Default is both.")
    parser.add_argument("--jk_mode", type=str, default="max",
                        choices=["last", "concat", "max", "sum", "mean"],
                        help="Jumping Knowledge mode. 'last' uses last layer only. Options: concat, max, sum.")
    parser.add_argument("--mask_ratio", type=float, default=0.3,
                    help="Ratio of feature dimensions to randomly mask for feature masking loss.")
    parser.add_argument("--feature_masking_weight", type=float, default=0,
                    help="Weight for the feature masking consistency loss (MSE between student outputs). Set to 0 to disable.")
    parser.add_argument("--residual", type=bool, default=False,
                    help="Use ResNet-style residual connections between GNN layers (default: False)")
    parser.add_argument('--alpha', type=float, default=0.1, help='GCNII alpha (initial residual weight)')
    parser.add_argument('--theta', type=float, default=0.5, help='GCNII theta (identity mapping strength)')
    parser.add_argument('--batch_size', type=int, default=16, 
                    help='Batch size for DataLoader, smaller values use less memory but process slower. Default is 128.')
    parser.add_argument('--train_model', type=bool, default=False, help='Train the model before inference. If false, only inference and evaluation will be performed.')
    parser.add_argument('--regenerate_embeddings', type=bool, default=False, help='Force regeneration of embeddings even if they already exist.')
