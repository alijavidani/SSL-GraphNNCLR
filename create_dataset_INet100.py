...
from adasim_utils.dataset import DatasetFolderAdaSim, ImageNetReturnPath
import subprocess
import tarfile
...
def train_adasim(args):
    ...
    # ============ preparing data ... ============
    transform = DataAugmentationDINO2(
    ...
    dataset_val = DatasetFolderAdaSim(args.data_path_val, args, transform=transform, return_index_instead_of_target=False)
    ...
    print(f"Data loaded: there are {len(dataset_val)} images.")
    ...
    node_features_train = torch.zeros(130000, 192, device='cuda')  # Assuming 192 is the feature dimension
    node_features_val = torch.zeros(5000, 192, device='cuda')  # Assuming 192 is the feature dimension

    y_train = torch.zeros(130000, dtype=torch.long, device='cuda')
    y_val = torch.zeros(5000, dtype=torch.long, device='cuda')
    
    for it, (images, targets, same_im_bool) in enumerate(metric_logger.log_every(data_loader, 10, header)):
    ...
    #             param_group["weight_decay"] = wd_schedule[it]
        indices = torch.tensor(np.arange(it * args.batch_size_per_gpu, (it + 1) * args.batch_size_per_gpu)).cuda(non_blocking=True)
    ...
    #         )
    #     reshaped_teacher = teacher_output.reshape(2, args.batch_size_per_gpu, -1).transpose(0, 1)[:,0,:]
    #     node_features_train[indices] = reshaped_teacher
        y_train[indices] = targets

    print(y_train)

    for it, (images, targets, same_im_bool) in enumerate(metric_logger.log_every(data_loader_val, 10, header)):
        # update weight decay and learning rate according to their schedule
        it = len(data_loader_val) * epoch + it  # global training iteration
        # for i, param_group in enumerate(optimizer.param_groups):
        #     param_group["lr"] = lr_schedule[it]
        #     if i == 0:  # only the first group is regularized
        #         param_group["weight_decay"] = wd_schedule[it]
        ...
        #     )
        # reshaped_teacher = teacher_output.reshape(2, args.batch_size_per_gpu, -1).transpose(0, 1)[:,0,:]
        # node_features_val[indices] = reshaped_teacher
        y_val[indices] = targets

    print(y_val)
    # node_features_teacher = torch.cat([node_features_train, node_features_val], dim=0)
    
    # num_repeats = 50000 // 10  # Equal numbers of 0s to 9s
    # y_train = torch.tensor([i for i in range(10) for _ in range(num_repeats)]).cuda()
    # y = torch.cat([y_train, y_val], dim=0)

    # teacher_graph_data = Data(x=node_features_teacher, y=y)
    # torch.save(teacher_graph_data, "./teacher_graph_data.pth")

    # teacher_graph_data = Data(x=node_features_teacher)
